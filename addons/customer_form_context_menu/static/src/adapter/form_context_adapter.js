...
/** Return the technical name of the field widget under the pointer. */
export function fieldNameFromTarget(formRoot, target) {
    const fieldWidget = target?.closest?.(".o_field_widget[name]");
    if (!fieldWidget || !formRoot?.contains?.(fieldWidget)) {
        return null;
    }
    const fieldName = fieldWidget.getAttribute("name");
    return typeof fieldName === "string" && fieldName.trim() ? fieldName : null;
}
...
export class FormContextAdapter {
    ...
    mount() {
        ...
        this.unregister = this.coordinator.registerForm(root, ({ event, anchor }) =>
            this.createContextSnapshot(anchor, event?.target)
        );
        return this.unregister;
    }
    ...
    createContextSnapshot(anchor, target = null) {
        ...
        return Object.freeze({
            ...
            actionId: controller.env?.config?.actionId || null,
            viewId: controller.env?.config?.viewId || null,
            selectedFieldName: fieldNameFromTarget(controller.rootRef?.el, target),
            pager: navigation,
            anchor,
