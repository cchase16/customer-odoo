...
test("accepts lazy translated strings and resolves them when building the model", () => {
    ...
    const group = new LazyTranslatedString("Fixture group");
    ...
    const model = menu.buildModel();
    expect(model[0].id).toBe("Translated Fixture group");
    expect(model[0].label).toBe("Translated Fixture group");
    const item = model[0].items[0];
